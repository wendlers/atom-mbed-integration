...
export default {
  ...
  consumeToolBar(toolbar) {
    ...
    if(!this.existsVirtualEnv() && this.supportedSystem()) {
      console.log('Ask to install mebd toolchain');
      this.showInstaller();
    }
  },
  ...
  showInstaller() {
    ...
    if(this.existsVirtualEnv()) {
      ...
      if(this.supportedSystem()) {
        ...
        m = '<p>It looks like there is no mbed toolchain installed yet. ' +
            'However, it looks like your system is not supported by the ' +
            'installer. Consider installing a mbed toolchain by hand.</p>' + m;        
      }
    }
